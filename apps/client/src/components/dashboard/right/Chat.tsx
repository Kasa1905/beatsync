...
export const Chat = () => {
  ...
  const getUserName = (deviceId: string, username: string) => {
    if (deviceId === currentUser?.clientId) return "You";
    return username;
  };
  ...
  const groupedMessages = messages.reduce((groups, msg, index) => {
    ...
    if (msg.deviceId === lastMsg.deviceId && isWithinTimeWindow) {
      lastGroup.push(msg);
    } else {
      groups.push([msg]);
    }
      ...
      >
        ...
        <div className="space-y-2">
          {groupedMessages.map((group, groupIndex) => {
            const isOwnMessage = group[0].deviceId === currentUser?.clientId;
